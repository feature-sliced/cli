...
describe.concurrent("the commands specified in usage examples", () => {
  for (const args of [
    ...
    test(
      `widget with segments (${argsSplit[0]})`,
      async () => {
        const project = join(
          temporaryDirectory,
        ...
        await expect(
          fs.stat(join(project, "src/entities")),
        ).rejects.toBeTruthy();
      },
      TIMEOUT,
    );
  ...
  for (const args of [
    ...
    const argsSplit = args.split(" ");
    test(
      `feature in a slice group (${argsSplit[0]})`,
      async () => {
        const project = join(
          temporaryDirectory,
  ...
  for (const args of ["e user -r ./src/lib", "entity user --root ./src/lib"]) {
    const argsSplit = args.split(" ");
    test(
      `entity in a relative non-existent root (${argsSplit[0]})`,
      async () => {
        const project = join(
          temporaryDirectory,
  ...
  for (const args of [
    ...
    const argsSplit = args.split(" ");
    test(
      `several pages with several segments (${argsSplit[0]})`,
      async () => {
        const project = join(
          temporaryDirectory,
  ...
  for (const [args, index] of [
    ...
    const argsSplit = args.split(" ");
    test(
      `shared with different segment syntaxes (${index})`,
      async () => {
        const project = join(
          temporaryDirectory,
